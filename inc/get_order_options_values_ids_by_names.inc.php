<?php
  ...
  {
    $ids_array = array('options_id'=>0,'value_id'=>0);
    ...
    $posible_query = xtc_db_query("SELECT products_options_id
                                    ...
                                    WHERE `products_options_name`='".xtc_db_input($options_name)."' AND language_id=".$language_id);
    if (xtc_db_num_rows($posible_query)) {
      ...
      if (!empty($options)) {
        $products_options_query = xtc_db_query("SELECT options_id,options_values_id 
                                                  ...
                                                  WHERE options_id IN (".implode(',', $options).") AND products_id=".(int)$pID);
        if (xtc_db_num_rows($products_options_query)) {
          ...
          $result = xtc_db_query(
            "
            SELECT
              pov2po.products_options_id AS options_id, pov.products_options_values_id AS value_id
            FROM
              ".TABLE_PRODUCTS_OPTIONS_VALUES." AS pov,
              ".TABLE_PRODUCTS_OPTIONS_VALUES_TO_PRODUCTS_OPTIONS." AS	pov2po
            WHERE
              pov2po.products_options_values_id IN(".implode(',', $options_values).") AND
              pov2po.products_options_values_id = pov.products_options_values_id AND
              pov.products_options_values_name 	= '".xtc_db_input($values_name)."' AND
              pov.language_id	= ".$language_id
          );
