...
function combine_files($f_array,$f_min,$compress_css = false,$f_time = 0)
{
    ...
    $needs_rebuild = ($f_min_ts === false || $compress || filesize($f_min_path) == 0 || $f_time > $f_min_ts);

    if ($sources_readable && $f_min_writable && $needs_rebuild) {
    ...
    } elseif (!$needs_rebuild && $f_min_ts !== false) {
      // A bundle that is stale but cannot be rebuilt (read-only deployment or
      // unreadable sources) must not be served; fall back to $f_array as-is.
      $f_array = array($f_min.'?v='.$f_min_ts);
    }
