...
// Ben�tigte Funktionen und Klassen Anfang:
...
if ($action == 'address_edit') {
  ...
  $sql_data_array = array (
      ...
      'customers_state' => xtc_db_prepare_input($_POST['customers_state']),
      ...
      'delivery_state' => xtc_db_prepare_input($_POST['delivery_state']),
      ...
      'billing_state' => xtc_db_prepare_input($_POST['billing_state']),
...
// Artikeldaten einf�gen / bearbeiten Anfang:
...
if ($action == 'product_edit') {
  ...
  //Produktpreise neu berechnen - Steuer hinzuf�gen
  if ($group_addtax) {
    $_POST['products_price'] += $_POST['products_price'] /100 * $product['products_tax'];
    //Optionspreise neu berechnen  - Steuer hinzuf�gen  //DEAKTIVIERT Optionspreise werden nur NETTO gespeichert???
  ...
  xtc_db_perform(TABLE_ORDERS, array('last_modified' => 'now()'), 'update', 'orders_id = '.(int)$_POST['oID']);
...
// Artikeldaten einf�gen / bearbeiten Ende:
...
if ($action == 'lang_edit') {
  ...
  $sql_data_array = array ('language' => xtc_db_prepare_input($lang['directory']), 'last_modified' => 'now()');
  xtc_db_perform(TABLE_ORDERS, $sql_data_array, 'update', 'orders_id  = \''.(int)($_POST['oID']).'\'');
...
if ($action == 'product_delete') {
  ...
  // Dokuman - 2010-03-17 - calculate stock correctly when editing orders
...
// L�schen eines Artikels aus der Bestellung Ende:

// L�schen einer Artikeloption aus der Bestellung Anfang:
...
// L�schen einer Artikeloptions aus der Bestellung Ende:

// L�schen eines OT Moduls aus der Bestellung Anfang:
if ($action == 'ot_delete') {
...
// L�schen eines OT Moduls aus der Bestellung Ende:

// L�schfunktionen Ende

// R�ckberechnung Anfang
...
if ($action == 'save_order') {
  ...
  //BOF - Web28 - 2011-01-16 -  L�schen des Zwischenspeichers
  xtc_db_query("DELETE FROM ".TABLE_ORDERS_RECALCULATE."
                      WHERE orders_id = '".(int)($_POST['oID'])."'");
  //EOF - Web28 - 2011-01-16 -  L�schen des Zwischenspeichers
  ...
  // Errechne neue Zwischensumme f�r Artikel Anfang
...
// Errechne neue Zwischensumme f�r Artikel Ende
  ...
  while ($module_value = xtc_db_fetch_array($module_query)) {
    ...
    if ($module_name != 'shipping' && $module_name != 'cod_fee' && $module_tax_rate == 0) { //web28 - 2012-03-14 - FIX Steuerberechnung
      ...
      $module_n_price -= $module_tax; //Korrektur f�r Nettosumme bei Modulen ohne Steuersatz
    }
  ...
  while ($ust = xtc_db_fetch_array($ust_query)) {
    ...
    //BOF web28 - 2010-12-04 - "inkl." oder "zzgl." hinzuf�gen
    ...
    //EOF web28 - 2010-12-04 - "inkl." oder "zzgl." hinzuf�gen
  ...
  //BOF  web28 - 2010-12-04 Errechne neue Gesamtsumme f�r Artikel
  //Mwst feststellen
  $add_tax = 0;
  $price = 'b_price';
  ...
  //EOF  web28 - 2010-12-04 Errechne neue Gesamtsumme f�r Artikel

  // L�schen des Zwischenspeichers
...
//Der Steuersatz muss anhand der Posten mit unterschiedlichen Steuers�tzen anteilig berechnet werden
function calculate_tax($amount) {
  ...
  //Steuers�tze alle Produkte der Bestellung feststellen
  $tax_rate_query = xtc_db_query("select tax_rate from ".TABLE_ORDERS_RECALCULATE." where orders_id = '".(int)$_POST['oID']."' and class = 'ot_tax' GROUP BY tax_rate");

  $tod_amount = 0;
  //Berechnungen pro Steuersatz durchf�hren
  while ($tax_rate = xtc_db_fetch_array($tax_rate_query)) {
    ...
    //Einzelne Steuers�tze neu in Kalkulationstabelle speichern
