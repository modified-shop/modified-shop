...
<script>
  var debug = false;
  var ajax_url = "backup_db.php?ajax=1&action=readdb&<?php echo xtc_session_name() . '=' . xtc_session_id(); ?>";
  var ajax_type = 'POST';
  var dataStr = '';
  ...
  function JStoPHPResponse(data) {
    ...
    if (response.nr < response.num_tables) {
      ...
      var button_back = '<a href="backup.php?file=' + file + '&<?php echo xtc_session_name() . "=" . xtc_session_id(); ?>" class="button"><?php echo BUTTON_BACK;?></a>';
