...
function add_table_infos_ibillnr($order)
{
    ...
    if (defined('MODULE_INVOICE_NUMBER_STATUS') && MODULE_INVOICE_NUMBER_STATUS == 'True') {
        ...
        $html .= '  <td class="main">'.($order->info['ibn_billnr'] == '' ? NOT_ASSIGNED : $order->info['ibn_billnr']).'</td>'.PHP_EOL;
        $html .= '</tr>'.PHP_EOL;
        $html .= '<tr>'.PHP_EOL;
        $html .= '  <td class="main"><b>'.ENTRY_INVOICE_DATE.'</b></td>'.PHP_EOL;
        $html .= '  <td class="main">'. ($order->info['ibn_billdate'] == '0000-00-00'? NOT_ASSIGNED : xtc_date_short($order->info['ibn_billdate'])).'</td>'.PHP_EOL;
        $html .= '</tr>'.PHP_EOL;
    }
    return $html;
}
...
function add_btn_ibillnr($order,$oID)
{
    ...
    if (defined('MODULE_INVOICE_NUMBER_STATUS') && MODULE_INVOICE_NUMBER_STATUS == 'True') {
      if ($order->info['ibn_billnr'] == '') {
        $html = '              <a class="button" href="'.xtc_href_link(FILENAME_ORDERS, 'page='.$_GET['page'].'&oID='.$oID.'&action=edit&action2=set_ibillnr').'">'. BUTTON_BILL .'</a>'.PHP_EOL;
      }
    }
    return $html;
}
