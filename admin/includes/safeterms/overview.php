...
/* --------------------------------------------------------------
   ...
   --------------------------------------------------------------*/
	...
	if ($safeterms_control["API_LASTVALID_RESULT"] == "TRUE") { 
		...
		$shop_update.= xtc_draw_form('safeterms_update', '', '', 'post') .'<input type="hidden" name="action" value="start_update" /><input type="submit" value="Update Starten" style="width:300px; height:40px;margin-left:50px; margin-top:15px; font-size:22px; background-color:#dedede; cursor:pointer;"; /></form></p>';
		...
		$shop_status = '			<p>Sie sind bereits Kunde bei Safeterms.de. Dann richten Sie jetzt das Auto-Update f&uuml;r Rechtstexte von Safeterms f&uuml;r Ihre modified eCommerce Shopsoftware ein.</p>
			...
			'.xtc_draw_form('auto_update', '', '', 'post').'
