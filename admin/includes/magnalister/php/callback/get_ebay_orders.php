...
function variation_products_model2pOpt($variation_products_model, $products_id) {
	...
	if ('artNr' != getDBConfigValue('general.keytype', '0')) $variation_attributes_select .= 
	    ' WHERE REPLACE(variation_products_model,\'ML\',\'\') = \''.str_replace('ML','',$variation_products_model).'\'';
						...
						if ((!empty($attrValues[0]['options_name']))
							...
							$MagnaDB->query('
								UPDATE '.TABLE_MAGNA_VARIATIONS. '
								 ...
								 WHERE variation_products_model = \''.MagnaDB::gi()->escape($prodOrderData['products_model']).'\'
							');
						}
					}
