<?PHP
  ...
  class haendlerbund_importer{
	...
	if(count($check_array) == 0 && $api_config == 1) {
        ...
        if(strtolower($_SESSION['language_charset']) != "utf-8") $return = utf8_encode($return);
    ...
    function saveConfiguration(){
      foreach($_POST as $configuration_key=>$configuration_value){
        ...
        if(mysql_num_rows($sql)) mysql_query("UPDATE configuration SET configuration_value='".$configuration_value."' WHERE configuration_key='".$configuration_key."' LIMIT 1");
		  }
      }      
    }
 ...
 $key= str_replace("_hbon","",$agb["configuration_key"]);
    ...
    function getContentIDSelect($label, $name, $id, $required=true, $lang= 1 ){
      ...
      while($row = mysql_fetch_object($sql)){
	   ...
	   if($row->content_id==$value) $selected=" selected='selected'";
