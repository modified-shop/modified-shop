...
if (!class_exists('ot_billpaytc_surcharge')) {
    class ot_billpaytc_surcharge
    {
        ...
        function _process() {
            ...
            $calculation = $_SESSION['bp_rate_result']['rateplan'][$_SESSION['bp_rate_result']['duration']]['calculation'];
            $surcharge 		= round($calculation['surcharge'] / 100, PRICE_PRECISION);
            $transactionFee = round($calculation['fee'] / 100, PRICE_PRECISION);
            $feeTax			= round($this->calculateTax($calculation['fee']) / 100, PRICE_PRECISION);
            $total          = round($calculation['total'] / 100, PRICE_PRECISION);
