...
/* -----------------------------------------------------------------------------------------
  ...
  if ($addresses_count > 0) {
    ...
    while ($addresses = xtc_db_fetch_array($addresses_query)) {
      ...
      // eindeutige ID, da mehrere Adressformulare auf der Seite stehen
      $address_content_array[$i]['FORM_ACTION'] = xtc_draw_form('checkout_address_'.$addresses['address_book_id'], xtc_href_link(((isset($billto) && $billto) ? FILENAME_CHECKOUT_PAYMENT_ADDRESS : FILENAME_CHECKOUT_SHIPPING_ADDRESS), $params, 'SSL'), 'post', 'class="checkout_address"').secure_form('checkout_address').xtc_draw_hidden_field('address', $addresses['address_book_id']);
