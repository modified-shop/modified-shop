...
final class LegacyApiDeprecationReporter
{
    ...
    private function caller(): ?array
    {
        ...
        $catalogRoot = FilesystemPath::canonicalize(DIR_FS_CATALOG)
            ?? FilesystemPath::normalize(DIR_FS_CATALOG);
        ...
        foreach (debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 8) as $frame) {
            ...
            $normalizedFile = FilesystemPath::canonicalize($file) ?? FilesystemPath::normalize($file);
            if (FilesystemPath::isWithin($normalizedFile, $catalogRoot)) {
                $normalizedFile = ltrim(substr($normalizedFile, strlen($catalogRoot)), '/');
            }
    ...
    {
        $directory = FilesystemPath::join(DIR_FS_CATALOG, self::RATE_LIMIT_DIRECTORY);
