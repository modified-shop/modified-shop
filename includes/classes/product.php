...
class product {
  ...
  * Constructor
  ...
  function getBuyNowButton($id, $name) {
    global $PHP_SELF;
    return '<a rel="nofollow" href="'.xtc_href_link(basename($PHP_SELF), xtc_get_all_get_params(array ('action')).'action=buy_now&BUYproducts_id='.$id, 'NONSSL').'">'.xtc_image_button('button_buy_now.gif', TEXT_BUY.$name.TEXT_NOW).'</a>';
  }
