...
if (is_object($order) && in_array($order->info['payment_method'], $paypal_payment_method)) {
  ...
  if (Template::findPath('module/paypal/payment_info.html') !== null) {
    $tpl_file = Template::path('module/paypal/payment_info.html');
  }
  ...
  if (strpos($order->info['payment_method'], 'link') !== false) {
    ...
    $paypal_payment_info = array(
      array ('title' => payment::payment_title($order->info['payment_method'], $order->info['order_id'], $order->info['language']).': ', 
             'class' => $paypal->code,
