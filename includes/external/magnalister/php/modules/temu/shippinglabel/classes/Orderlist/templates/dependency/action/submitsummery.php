...
?>
<a id="ml-confirm-shipping" title="<?php echo ML_TEMU_SHIPPINGLABEL_CONFIRM; ?>"  href="<?php echo $this->getUrl(false, false, false, array('view' => 'upload', 'subview'=>'form')); ?>" class="ml-js-noBlockUi ml-button mlbtn-action"><?php echo ML_TEMU_SHIPPINGLABEL_CONFIRM; ?></a> 
   <?php ob_start();?>
...
$(document).ready(function() {
	$('#ml-confirm-shipping').click(function(e){
		...
		$('<?php echo $popup ?>').jDialog({
			title: '<?php echo ML_TEMU_SHIPPINGLABEL_CONFIRM; ?>',
			buttons: [
			{
...
//$(document).ready(function() {
	function runConfirmShippingAjax() {
		...
		csaj.addLocalizedMessages({
			...
			'MessageUploadFinal': <?php echo json_encode(ML_TEMU_SHIPPINGLABEL_SUMMARY_STATISTIC); ?>,
