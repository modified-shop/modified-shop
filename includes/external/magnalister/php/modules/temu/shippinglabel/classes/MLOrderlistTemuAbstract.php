...
abstract class MLOrderlistTemuAbstract {
...
//		$this->hookAddOrderlistDependencies();
	...
	/**
	 * @return MLOrderlistTemuAbstract Description
	 */
	...
	protected function addDependency($sDependencyName, $aDependencyConfig = array()) {
		...
		require_once(DIR_MAGNALISTER_MODULES_TEMU_ORDERLIST . '/Dependency/' . $sDependencyName . '.php');
	...
	public function renderTemplate() {
		...
		include DIR_MAGNALISTER_MODULES_TEMU_ORDERLIST . '/templates/' . func_get_arg(0) . '.php';
...
//		echo print_m($aResult, 'result');
	...
	protected function getSelectionData($aRow = null, $sFieldName = null) {
		if($aRow !== null) {
			...
			// $aRow may be an arbitrary order-list row (e.g. from isInSelection()) that does
			// not carry the selection key — treat that as "not selected" instead of emitting
			// an undefined-key / array-offset-on-non-array warning.
			if (!is_array($aRow) || !isset($aRow[$sSelectionKey])) {
				return ($sFieldName === null) ? array() : null;
			}
			$mKey = $aRow[$sSelectionKey];
			if (!isset($this->aSelectionData[$mKey])) {
				// $mKey originates from request input (POST/GET keys) — escape it.
				$aSelection = MagnaDB::gi()->fetchRow("
					SELECT *
					FROM " . $this->getSelectionTableName() . "
					WHERE
						`element_id` = '" . MagnaDB::gi()->escape($mKey) . "'
						AND `session_id` = '" . session_id() . "'
						AND `mpID` = '" . $this->aMagnaSession['mpID'] . "'
						AND `selectionname` = '" . $this->getSelectionName() . "'
				");
				// fetchRow() returns false when no selection row exists — normalize to an
				// array and decode the JSON `data` column exactly once, on the cache miss.
				if (!is_array($aSelection)) {
					$aSelection = array();
				} elseif (isset($aSelection['data'])) {
					$aSelection['data'] = json_decode($aSelection['data'], true);
				}
				$this->aSelectionData[$mKey] = $aSelection;
			}
			if ($sFieldName === null) {
				return $this->aSelectionData[$mKey];
			} else {
				return isset($this->aSelectionData[$mKey][$sFieldName]) ? $this->aSelectionData[$mKey][$sFieldName] : null;
			}
		}  else {
