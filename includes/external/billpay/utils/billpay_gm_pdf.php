...
/* -----------------------------------------------------------------------------------------
   ...
   ---------------------------------------------------------------------------------------*/
	...
	if ($order->info['payment_method'] == 'billpay') {
 		$bank_data_query = xtc_db_query("SELECT account_holder,
 		                                        account_number,
 		                                        bank_code,
 		                                        bank_name,
 		                                        invoice_reference,
		...
		if (xtc_db_num_rows($bank_data_query) > 0) {
			...
			//$sBankDataInfo = decode_htmlentities(MODULE_PAYMENT_BILLPAY_TEXT_INVOICE_INFO1 . "$day.$mon.$year" . MODULE_PAYMENT_BILLPAY_TEXT_INVOICE_INFO2);
			$sBankDataInfo = sprintf(decode_htmlentities(MODULE_PAYMENT_BILLPAY_TEXT_INVOICE_INFO), $bank_data['invoice_reference'], $day, $mon, $year);
			$sBankData1 = decode_htmlentities(MODULE_PAYMENT_BILLPAY_TEXT_ACCOUNT_HOLDER) . ':';
			$sBankData2 = decode_htmlentities(MODULE_PAYMENT_BILLPAY_TEXT_ACCOUNT_NUMBER) . ':';
			$sBankData3 = decode_htmlentities(MODULE_PAYMENT_BILLPAY_TEXT_BANK_CODE) .':';
			$sBankData4 = decode_htmlentities(MODULE_PAYMENT_BILLPAY_TEXT_BANK_NAME) .':';
			$sBankData5 = decode_htmlentities(MODULE_PAYMENT_BILLPAY_TEXT_PURPOSE) .':';
			$sBankData6 = decode_htmlentities(MODULE_PAYMENT_BILLPAY_DUEDATE_TITLE) .':';
			$y = parent::GetY();
	...
	} elseif ($order->info['payment_method'] == 'billpaydebit') {
		require_once(DIR_FS_LANGUAGES . $_SESSION['language'] . '/modules/payment/billpaydebit.php');
		$iConstantMargin = 40;
		$sBankDataInfo = decode_htmlentities(MODULE_PAYMENT_BILLPAYDEBIT_TEXT_INVOICE_INFO1 . ' ' . MODULE_PAYMENT_BILLPAYDEBIT_TEXT_INVOICE_INFO2);
		$y = parent::GetY();
		...
		$get_y = $this->getActualY($y);
		parent::SetY($get_y + 5);
	} elseif ($order->info['payment_method'] == 'billpaytransactioncredit') {
		...
		$activated_query = xtc_db_query("SELECT invoice_due_date
		                                   FROM billpay_bankdata
		                                  WHERE orders_id = '".$_GET['oID']."'");
		...
		parent::MultiCell(parent::getInnerWidth(), parent::getCellHeight(), decode_htmlentities(strip_tags(MODULE_PAYMENT_BILLPAYTRANSACTIONCREDIT_TEXT_INVOICE_INFO1)), '', 'L', 0);
		parent::SetFont($header_font_type, $header_font_style, 7);
		$get_y = $this->getActualY($y);
		parent::SetY($get_y + 2);

		$rate_details_query = xtc_db_query("SELECT rate_surcharge,
		                                           rate_total_amount,
		                                           rate_count,
		                                           rate_dues,
		                                           rate_interest_rate,
		                                           ...
		                                           rate_fee_tax
		                                      FROM billpay_bankdata
		                                     WHERE api_reference_id = '".$_GET['oID'] . "'");
		...
		foreach ($dueDateArray as $entry) {
			...
			/*parent::MultiCell(parent::getInnerWidth(), parent::getCellHeight(), decode_htmlentities(strip_tags($rateCount.' '.MODULE_PAYMENT_BILLPAYTRANSACTIONCREDIT_TEXT_RATE)), '', 'L', 0);
			$get_x = 10;
			parent::SetX($get_x + 20);
			parent::MultiCell(parent::getInnerWidth(), parent::getCellHeight(), decode_htmlentities(strip_tags(xtc_format_price_order($entry['value'] / 100, 1, $order->info['currency']))), '', 'L', 0);
			...
			if(count($dueDateArray) >= 12) {
				...
				if (!empty($entry['date'])) {
					parent::Cell(60, 4, '('.decode_htmlentities(MODULE_PAYMENT_BILLPAYTRANSACTIONCREDIT_RATEDUE_TEXT .' '. xtc_date_short($trimmedDueDate)).')', '', '');
				}
				if($rateCount == (count($dueDateArray) / 2)) {
					parent::SetY($b_y);
				}
				...
				if (!empty($entry['date'])) {
					parent::Cell(20, 4, '('.decode_htmlentities(MODULE_PAYMENT_BILLPAYTRANSACTIONCREDIT_RATEDUE_TEXT .' '. xtc_date_short($trimmedDueDate)).')', '', '');
				}
				parent::ln(4);
			}
			++$rateCount;
		...
		parent::Cell(20, 4, ' '.decode_htmlentities(MODULE_PAYMENT_BILLPAYTRANSACTIONCREDIT_TOTAL_PRICE_CALC_TEXT), '', '');
		parent::SetFont($header_font_type, $header_font_style, 7);
		parent::ln(4);
		parent::Cell(20, 4, ' '.decode_htmlentities(MODULE_PAYMENT_BILLPAYTRANSACTIONCREDIT_CART_AMOUNT_TEXT).'='.xtc_format_price_order($data['rate_base_amount'], 1, $currency), '', '');
		parent::ln(4);
		parent::Cell(20, 4, ' '.decode_htmlentities(MODULE_PAYMENT_BILLPAYTC_SURCHARGE_TEXT), '', '');
		parent::ln(4);
		...
		parent::ln(4);
		parent::Cell(20, 4, ' '.decode_htmlentities(MODULE_PAYMENT_BILLPAYTRANSACTIONCREDIT_TRANSACTION_FEE_TEXT).$taxString.'+'.xtc_format_price_order($data['rate_fee'], 1, $currency), '', '');
		parent::ln(4);
		parent::Cell(20, 4, ' '.decode_htmlentities(MODULE_PAYMENT_BILLPAYTRANSACTIONCREDIT_OTHER_COSTS_TEXT).'+'.xtc_format_price_order($otherCosts, 1, $currency), '', '');
		parent::ln(4);
		parent::Cell(20, 4, ' '.decode_htmlentities(MODULE_PAYMENT_BILLPAYTRANSACTIONCREDIT_TOTAL_AMOUNT_TEXT).'='.xtc_format_price_order($data['rate_total_amount'], 1, $currency), '', '');
		parent::ln(4);
		parent::Cell(20, 4, ' '.decode_htmlentities(MODULE_PAYMENT_BILLPAYTRANSACTIONCREDIT_ANUAL_RATE_TEXT).'='.round($data['rate_anual_rate'], 2).'%', '', '');
		parent::ln(4);
		parent::ln(4);
 		//parent::Cell(20, 4, ' '.MODULE_PAYMENT_BILLPAYTRANSACTIONCREDIT_EXAMPLE_TEXT, '', '2');
 		parent::MultiCell(parent::getInnerWidth(), parent::getCellHeight(), decode_htmlentities(strip_tags(MODULE_PAYMENT_BILLPAYTRANSACTIONCREDIT_EXAMPLE_TEXT)), '', 'L', 0);
		parent::ln(4);
		parent::ln(4);
	}
?>