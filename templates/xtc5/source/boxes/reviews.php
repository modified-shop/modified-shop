...
if ($product->isProduct() === true && $_SESSION['customers_status']['customers_status_write_reviews'] == '1') {
  ...
  if (xtc_db_num_rows($reviews_query) > 0) {                  
    ...
    if (!$box_smarty->is_cached(Template::resolve('boxes/box_reviews.html'), $cache_id) || !$cache) {
      ...
      $review_image = xtc_image(Template::url('img/stars_' . $reviews['reviews_rating'] . '.gif') , sprintf(BOX_REVIEWS_TEXT_OF_5_STARS, $reviews['reviews_rating']));
      $review_image_microtag = xtc_image(Template::url('img/stars_' . $reviews['reviews_rating'] . '.gif') , sprintf(BOX_REVIEWS_TEXT_OF_5_STARS, $reviews['reviews_rating']),'','','itemprop="rating"');
