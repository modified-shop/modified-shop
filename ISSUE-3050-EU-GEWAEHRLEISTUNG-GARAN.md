...
- Das Aufraeumen loescht ausschliesslich, schreibt nie und fragt den Katalog gar nicht. Es vergleicht zwei Zahlen der Bestellung selbst: die verbliebenen Attribute der Position und ihre Downloadzeilen.
  - Kein Attribut mehr vorhanden: Jede Zeile ist verwaist, alle werden entfernt.
  - Mehr Zeilen als Attribute: Mindestens eine Zeile ist verwaist, welche das ist steht nicht fest. Es wird nichts geloescht und der Shopbetreiber erhaelt eine Meldung ueber `messageStack->add_session()`, weil `admin/orders_edit.php` anschliessend weiterleitet.
  - Sonst: Jede Zeile kann noch zu einem verbliebenen Attribut gehoeren, es geschieht nichts.
...
- Eine verwaiste Zeile ist der harmlose Fehler, ein entzogenes Downloadrecht nicht. Die Folge fuer GARAN ist begrenzt: Eine Position mit einer verbliebenen Zeile gilt weiterhin als digital, der Gewaehrleistungshinweis kann in diesem Randfall also entfallen, wo er haette erscheinen duerfen. Die Meldung nennt den Fall, damit der Shopbetreiber ihn von Hand klaeren kann.
...
- Ein Attribut entfernen, sodass Zeilen und Attribute gleich viele bleiben; es wird weder geloescht noch gewarnt.
