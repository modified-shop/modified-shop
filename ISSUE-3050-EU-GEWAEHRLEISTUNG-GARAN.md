...
- Ein Hersteller, den der Import selbst anlegt, ist inaktiv: `admin/includes/classes/import.php` schreibt beim Anlegen kein `manufacturers_status`, und die Spalte hat keinen Standardwert. Traegt dieselbe Zeile eine Garantiedauer, lehnt die Pruefung sie deshalb ab und meldet das je Zeile. Das ist beabsichtigt, ein Label mit einem inaktiven Hersteller entstuende sonst ebenfalls nicht. Der Shopbetreiber schaltet den Hersteller unter Hersteller aktiv und importiert die Zeile erneut.
...
- Jede Aenderung wird mit Zeitpunkt, Admin-Benutzer sowie alten und neuen Werten in der Bestellhistorie protokolliert. Die Garantiebedingungen gehen dabei mit Dateiname und den ersten acht Zeichen ihres Inhaltshashes ein. Ein Dokument heisst nach seinem Inhalt: Wird es unter demselben Dateinamen ersetzt, aendert sich nur `terms_hash`, und ein Vergleich der Namen liesse die Korrektur spurlos durchgehen.
...
- fehlende oder beschaedigte historische Archivdateien, gezaehlt ueber jede Datei, die ein Leseweg braucht.
...
- Archivverzeichnis mit fehlender `nested.svg` oder `notice.json` sowie mit abweichender Pruefsumme anlegen; beide Faelle zaehlen als beschaedigt. Die Diagnose zaehlt ein solches Verzeichnis auch dann, wenn die erste Datei vorhanden ist.
...
- Garantiebedingungen durch eine geaenderte Fassung unter demselben Dateinamen ersetzen; die Historie nennt beide Fassungen mit ihrem Kurzhash, obwohl Dateiname, Hersteller, Modellkennung und Dauer gleich bleiben.
