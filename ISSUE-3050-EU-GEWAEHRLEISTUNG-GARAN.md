...
| `xtc_cfg_multi_checkbox()` | ruft den benannten Helfer und stellt jede gelieferte Zeile dar | Die Adminguppe erscheint in der Liste. Der Beschreibungstext sagt, dass sie ignoriert wird. |
...
- Die Anzahl der Herstellerabfragen richtet sich nach der Ausgabestelle. Wo der ganze Ergebnisblock vorliegt, genuegt eine Abfrage: Produktdetailseite, Warenkorbdetail und Merkzettel sammeln die Hersteller ihrer Positionen einmal vorab. Artikellisten ueber `buildDataArray()` und Bestellpositionen ueber `getOrderData()` sehen je Aufruf nur eine Zeile; dort bleibt es bei einer Abfrage je unterschiedlichem Hersteller der Seite. Fuehrt jeder Artikel einen eigenen Hersteller, entspricht das einer Abfrage je Artikel.
