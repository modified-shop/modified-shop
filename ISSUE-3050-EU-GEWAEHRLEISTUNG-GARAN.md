...
- Das Aufraeumen loescht ausschliesslich, schreibt nie und fragt den Katalog gar nicht:
  ...
  - Kein Downloadattribut mehr: Jede Zeile ist verwaist, alle werden entfernt.
  - Mehr Zeilen als Downloadattribute: Mindestens eine Zeile ist verwaist, welche das ist steht nicht fest. Es wird nichts geloescht und der Shopbetreiber erhaelt eine Meldung ueber `messageStack->add_session()`, weil `admin/orders_edit.php` anschliessend weiterleitet.
  - Sonst: Jede Zeile kann noch zu einem verbliebenen Downloadattribut gehoeren.
- Ein Downloadattribut erkennt der Shop am Namen der Option. `orders_products_attributes.products_options` haelt diesen Namen als Text der Bestellung fest, nicht des Katalogs, und die Attributverwaltung bietet die Downloadfelder nur fuer eine Option namens `Downloads` an. Der Name steht dafuer unmittelbar in der Abfrage und nicht als gemeinsame Konstante: Er soll verschwinden, nicht etabliert werden, und `admin/includes/modules/new_attributes_include.php` bleibt unveraendert.
- Damit ist der haeufigste Fall geloest: Wird das einzige Downloadattribut einer Position geloescht, die auch ein koerperliches traegt, bleibt kein Downloadattribut uebrig und die Zeile verschwindet. Ein blosses Zaehlen aller Attribute haette hier ein Attribut neben einer Zeile gesehen und nichts getan.
- Der Weg ueber den Optionsnamen ist eine Zwischenloesung. Er traegt, solange die Attributverwaltung die Downloadfelder an diesen Namen bindet; benennt ein Shopbetreiber das Attribut einer Bestellung von Hand um, landet der Fall in der meldenden Verzweigung. Issue #3280 ersetzt ihn durch einen direkten Verweis der Downloadzeile auf ihr Attribut und macht den Namen entbehrlich.
...
- Eine verwaiste Zeile ist der harmlose Fehler, ein entzogenes Downloadrecht nicht. Bleibt eine Zeile stehen, gilt die Position weiterhin als digital und der Gewaehrleistungshinweis entfaellt dort, wo er haette erscheinen muessen. Nach der Erkennung ueber den Optionsnamen trifft das nur noch Positionen, deren Attributbezeichnung von Hand geaendert wurde; dort erscheint die Meldung und der Shopbetreiber kann den Fall klaeren.
...
- Das einzige Downloadattribut einer Position entfernen, die auch ein koerperliches Attribut traegt; die Downloadzeile verschwindet, es erscheint keine Warnung und die Bestellung gilt danach als koerperlich.
- Ein koerperliches Attribut aus einer Position ohne Downloadzeilen entfernen; es erscheint keine Warnung.
- Die Bezeichnung eines Downloadattributs in der Bestellung von Hand aendern und danach ein Attribut loeschen; die Zeile bleibt stehen und die Warnung erscheint.
...
Der Umsetzungsstand behilft sich mit dem Namen der Option: Ein Downloadattribut heisst `Downloads`, weil die Attributverwaltung die Downloadfelder nur dort anbietet. Das loest den haeufigsten Fall, haengt aber an einer Bezeichnung, die ein Shopbetreiber in der Bestellung aendern kann. Bleiben dann mehr Downloadzeilen als erkannte Downloadattribute uebrig, wird keine entfernt und gemeldet; die Alternative wuerde Downloadrechte von Kunden entziehen oder erfinden.
...
Die Entscheidung ist getroffen: Der Stand bleibt bei der Erkennung ueber den Optionsnamen. Die exakte Loesung ist als Issue #3280 festgehalten. Fuer GARAN bleibt damit nur der Fall offen, dass die Attributbezeichnung einer Bestellung von Hand geaendert wurde; dann kann der Gewaehrleistungshinweis entfallen und der Shopbetreiber erhaelt eine Meldung.
