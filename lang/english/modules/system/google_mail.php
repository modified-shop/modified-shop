...
/* -----------------------------------------------------------------------------------------
  ...
  define('MODULE_GOOGLE_MAIL_TEXT_CONNECTION_ERROR', 'OAuth connection error: <b>%s</b>. Please reconnect.');
  ...
  define('MODULE_GOOGLE_MAIL_TEXT_MISSING_CREDENTIALS', 'Please enter and save the OAuth Client ID, Client Secret and Gmail address before connecting.');
...
<li>Under <b>APIs &amp; Services &rarr; OAuth consent screen</b>: choose <b>Internal</b> for a Google Workspace account (no verification needed), or <b>External</b> for a personal @gmail.com account. Add the scope <code>https://mail.google.com/</code>. For External + Testing status, add the Gmail address as a test user - note that Testing-status refresh tokens expire after 7 days. The scope required for SMTP is restricted; Google may require verification for a public External app and may require send-only apps to use the Gmail API with <code>gmail.send</code> instead. A Workspace/Internal account is recommended for internal use.</li>
