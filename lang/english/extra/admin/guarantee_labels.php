...
/* -----------------------------------------------------------------------------------------
  ...
  define('TEXT_GUARANTEE_LABELS_INFO_RULES', 'A GARAN label starts at 2.5 years. Shorter periods are stored and record what the manufacturer grants; they produce no label, because a guarantee of up to two years gives the customer nothing on top of the legal guarantee. A label also requires the guarantee to be free of charge, to cover the whole product and to apply with the same duration and the same conditions in every country you deliver to. An empty field means that nothing is recorded about a durability guarantee.');
  ...
  define('ERROR_GUARANTEE_LABELS_TERMS_BOTH', 'GARAN guarantee conditions: a file and an external link together are two different documents. The product page shows the link, the mail sends the file. Remove the link. The attachment was saved as an ordinary one.');
  define('ERROR_GUARANTEE_LABELS_SNAPSHOT_FAILED', 'GARAN: the order data could not be stored completely: %s');
  define('ERROR_GUARANTEE_LABELS_ORDER_ARCHIVE', 'GARAN: reading the archived order data failed: %s');
